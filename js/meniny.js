...
function vypisDatum() {
    ...
    xmlDoc = xmlhttp.responseXML;
        ...
        if(flag) {
            ...
            alert(output + " má meniny: " + prekonvertujDatum(x));  
        }
        else {
            //document.getElementById("datumPreMeno").innerHTML = meno + " sa v kalendári nenachádza.";
            alert(meno + " sa v kalendári nenachádza.");
        }
}
        ...
        for(var i=0;i<temp.length;i++) {
                ...
                if(najdenyDen.localeCompare(x)==0) {
                        try {  
                            ...
                            alert(prekonvertujDatum(x) + " má meniny: " + temp[i].getElementsByTagName("SK")[0].childNodes[0].nodeValue);
                            break;
                        }
                        catch(err) {
                            ...
                            alert(prekonvertujDatum(x)+" je sviatok: " + temp[i].getElementsByTagName("SKsviatky")[0].childNodes[0].nodeValue);
                            break;
                        }
